(function (window) {
	 ...
	 */
	SettingsView.prototype.bind = function (command, handler) {
		var self = this;
		if (command === 'playClick') {
		...
		} else if (command === 'difficultyVisibility') {
			self._bindModeEvent();
		} else {
			throw new Error('Unknown View command! Command is ' + command);
		}
	};
